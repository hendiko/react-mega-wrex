 ...
 */
import ReactMegaWrex from './types/index.d';
import { isPlainObject } from './utils';

/** 核心 reduce 函数，实现状态的合并、重置、清空 */
const storeReducer: ReactMegaWrex.WrexReactReducer = (state, action) => {
  const { type, payload } = action || {};
  // support three types which specify how to change sotre states.
  if (type === 'merge')
    return state === payload ? state : { ...state, ...payload };
  if (type === 'reset')
    return (isPlainObject(payload) ? payload : {}) as ReactMegaWrex.WrexState;
  if (type === 'clear') return {};
  return state;
};

/** 更新异步 reduce 函数运行的 loading 状态 */
const loadingReducer: ReactMegaWrex.WrexLoadingReducer = (state, action) => {
